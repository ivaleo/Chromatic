...
def test_proven_r9_step_is_documented():
    """Вытесненное 9604 остаётся в тексте как аналитический путь: 6/7+1/9=61/63.

    Само 9604 доказано продуктовым исчислением и верно; с 07.09.2026 оно
...
def test_dim9_7203_exact_data_is_consistent():
    """Данные сертификата R^9: подрешётка индекса 7203, Грам симметричен и PD.

    С 07.09.2026 оценка доказана (ssec:7203): перечисление 1 654 230 вершин
    девятимерной ячейки доведено до конца после точной unimodular-замены
    базиса, и 7203 стоит в заголовке вместо 9604.
    """
    ...
    # рационализация огрубляется в координатах базы, а не в декартовых:
    # у пройденной точки знаменатель 10^4 и целые до 30000 — на порядок
    # мельче, чем дало бы округление декартова сдвига
    biggest = max(abs(x) for row in C.INTEGER_GRAM for x in row)
    assert biggest <= 100_000

    # верхний блок 8x8 — это E8 при lambda_1^2 = 3, определитель (3/2)^8
