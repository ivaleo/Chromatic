...
PROBES = [
    ...
    # посев --- сертифицированная точка семейства G(a) (dim3_k15), а не ОЦК:
    # у ОЦК ширина ровно 1 и лучшей известной решёточной 15-раскраской он не
    # является. sigma = 0.003 / 0.01 / 0.03, по 2500 вычислений; лучшее CMA ---
    # 1.0135771, то есть посев не превзойдён (но и не удержан: проба слабее
    # остальных строк, см. оговорку в статье).
    {"case": "R^3 / 15", "dim": 3, "colours": 15, "parameters": 62,
     "champion": 1.0265922463876391, "reached": 1.0265922463876391,
     "cma_best": 1.013577064238553, "holds_seed": False,
     "source": "G(3137/10000), Hermite [[1,0,11],[0,1,7],[0,0,15]]"},
...
def _g3137() -> np.ndarray:
    """Базис G(3137/10000) --- сертифицированная точка при k = 15 (dim3_k15)."""
    gram = np.array([[10000, -3137, -3726], [-3137, 10000, -3137],
                     [-3726, -3137, 10000]], float) / 10000.0
...
def verify_champions() -> list[dict]:
    ...
    for name, build, H, want in [
        ...
        ("R^3 / 15", _g3137, [[1, 0, 11], [0, 1, 7], [0, 0, 15]],
         1.0265922463876391),
