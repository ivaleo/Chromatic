...
ROOT = "/Users/mac/Documents/_My_code/Chromatic/audit-data"
...
if __name__ == "__main__":
    W45 = np.array(json.load(open(f"{ROOT}/n6_push45.json"))["Q"])
    W46 = np.array(json.load(open(f"{ROOT}/n4_push46.json"))["Q"])
    X44 = json.load(open(f"{ROOT}/n7_push44.json"))["k44"]["x"]
    ...
    with Pool(2) as pool:  # основная кампания занимает 8 ядер
        for r in pool.imap(work, jobs):
            ...
            json.dump(out, open(f"{ROOT}/n9_bk4d.json", "w"), indent=1)
