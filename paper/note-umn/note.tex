...
% (см. README.md: что сокращать, если редакция потребует ровно две).
...
% решётка индекса 43 и точный верификатор 1029 (Н.~Глушкова).
\author{Л.\,Л.~Иванов, Н.~Глушкова}
\date{}
...
$\chi(\R^9)\le9604$, $\chi(\R^{10})\le45619$. Точнее,
$\chi(\R^n,[1,\ell])\le k$ при $(n,k)=(4,43)$ для $\ell\le1{,}00411$,
при $(5,132)$ для $\ell\le101/100$, при $(7,1029)$ для $\ell\le103/100$,
при $(9,9604)$ для $\ell<\sqrt{63/61}$ и при $(10,45619)$ для
$\ell<\sqrt{217/214}$.
\end{theorem}

Оценка $45619$ --- первая в $\R^{10}$ ниже классической величины $3^n$.
Первые три оценки даны явными рациональными решётками с проверкой в точной
арифметике (п.~3), последние две выведены аналитически (п.~4); подробности
--- \cite{Full}.
...
Для трёх решёток ниже проверка выполнена целиком в точной арифметике; полные
данные --- в \cite{Full}.
...
$|v|<2(1+\tfrac{101}{100})R$ лежат $38$ векторов $\Gamma$,
{\small
\[
...
В каждом из трёх случаев \eqref{crit} даёт утверждение теоремы; для $n=4$ и
$n=7$ проверка независимо повторена второй программой без общих процедур.

\textbf{4. Продуктовое правило и размерности $9$, $10$.}
...
$N(5+2\omega)=19$. По предложению~1 $D(\Gamma_3)\ge2\sqrt{31/12}$, поэтому
$d_3\ge\sqrt{31/12}\cdot\sqrt3=\sqrt{31/4}$ и цена не превосходит $4/31$.

Применим лемму к $E_8\oplus t\Z$ с $\Gamma_1\oplus4t\Z$ и к
$E_8\oplus a\Z[\omega]$ с $\Gamma_1\oplus(5+2\omega)a\Z[\omega]$ при подходящих
$t,a>0$: так как $\frac67+\frac19=\frac{61}{63}<1$ и
$\frac67+\frac4{31}=\frac{214}{217}<1$, получаем
$\chi(\R^9,[1,\ell])\le2401\cdot4=9604$ при $\ell<\sqrt{63/61}$ и
...
% TODO: место работы обоих авторов и e-mail второго автора
\noindent
\parbox[t]{0.5\textwidth}{{\bf Л.\,Л.~Иванов (L.\,L.~Ivanov)}\\
Москва\\
{\it E-mail}: leo.ivanov@gmail.com}%
\parbox[t]{0.5\textwidth}{{\bf Н.~Глушкова (N.~Glushkova)}\\
---\\
{\it E-mail}: ---}
