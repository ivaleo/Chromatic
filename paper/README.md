# paper/ — статья об оценках χ(ℝ⁴) ≤ 43, χ(ℝ⁵) ≤ 132, χ(ℝ⁷) ≤ 1029, χ(ℝ⁹) ≤ 7203 и χ(ℝ¹⁰) ≤ 45619

*(в заголовке — только доказанные оценки, статусы [Т]/[С]; численно, со статусом [Ч],
получено более сильное 28812 в ℝ¹⁰ — см. шкалу статусов в конце введения)*

Комплект публикации. Основная статья — [`article1/`](article1/) (пять
доказанных оценок); полная рукопись [`chi4-43.tex`](chi4-43.tex) (PDF —
`chi4-43.pdf`) — электронное дополнение со всеми подробностями.

## Комплект публикации (с 03.09.2026)

Полная рукопись `chi4-43.tex` (69 страниц) остаётся **электронным
дополнением**; из неё выделены самостоятельные документы, каждый со своим
каркасом, разделами и README:
...
| [`note-dan/`](note-dan/) | краткое сообщение для Докладов РАН (`dan.tex`, 15.09.2026) | то же сообщение, площадка 2 | короткая статья на лимит 25 000 знаков: все пять оценок, четыре конструкции с данными, протокол точной проверки, вывод 45619 с доказательствами; стиль `dan2e.sty`, 5 страниц |
| [`arxiv-en/`](arxiv-en/) | короткая английская версия для arXiv (`bounds-en.tex`, 14.09.2026) | препринт: первой идёт на arXiv | все пять оценок с данными четырёх сертификатов, тождество, продуктовое правило, пол на E₈; 8 страниц, только английский |

Предисловие о роли полной версии — `split-preface.tex` (подключается перед
оглавлением). Инварианты разделения — `audit-data/tests/test_paper_split.py`.
...
| Файл | Что это |
|---|---|
| `chi4-43.tex` | Каркас статьи: преамбула, титул, `\input` секций, библиография. |
...
| `split-preface.tex` | Предисловие полной рукописи: её роль в комплекте публикации. |
| `arxiv-metadata.md` | Метаданные и чек-лист подачи на arXiv. |
| `chi4-43.pdf` | Собранная версия (для чтения без TeX). |
...
Все документы комплекта собирает `make paper` из корня репозитория; ниже —
сборка одной полной рукописи вручную.

```bash
cd paper

# (опционально) перегенерировать рисунки из данных экспериментов:
python figures.py            # читает ../audit-data/results/*.json → fig_*.pdf

# собрать PDF:
latexmk -pdf chi4-43.tex     # рекомендуется (сам прогонит нужное число раз)
# либо вручную:
#   pdflatex chi4-43.tex && pdflatex chi4-43.tex
```
