...
\midrule
$4$  & $49$ & $\mathbf{43}$ & $1.00411$ & Eisenstein lattice \hfill\stC\\
$4$  & $45$ & $45$ & $1.015$ & general-position lattice (wider) \hfill\stC\\
$5$  & $140$ & $\mathbf{132}$ & $1.010$ & general-position lattice \hfill\stC\\
$7$  & $1372$ & $1323$ & $1.007$ & general-position lattice \hfill\stC\\
$7$  & $1323$ & $\mathbf{1029}$ & $1.032878$ & lamination of $E_6^*/343$ \hfill\stC\\
$9$  & $17253$ & $\mathbf{7203}$ & $1.0166127$ & lamination of $E_8/2401$, exact \hfill\stC\\
$9$  & $17253$ & $9604$ & $\sqrt{63/61}$ & product calculus \hfill\stP\\
...
$24$ & \multicolumn{2}{l}{$7^{12}$ (known bound~\cite{ABPR})} & $<\sqrt{7/6}$ (exact width) & new width: Eisenstein identity \hfill\stP\\
...
carries only the five bounds with status \stP{} or \stC{} --- $43$, $132$,
$1029$, $7203$, $45619$; the sharper $28812$ has status
\stN{} and is stated as such throughout. In
...
(Section~\ref{ssec:wide45}), $\chi(\R^5,[1,\ell])\le132$ for $\ell\le101/100$,
and $\chi(\R^7,[1,\ell])\le1323$ for $\ell\le1007/1000$
(Section~\ref{sec:r57}), all with status \stC. The same exact discipline,
applied to the Voronoi cell of a laminated lattice, gives
$\chi(\R^7,[1,\ell])\le1029$ for $\ell\le103/100$
(Theorem~\ref{thm:r7ex}) --- the bound that goes into the title. A byproduct is a
colors-versus-width trade-off in $\R^4$: $46$ colors for $\ell\le1.004$ and
$48$ colors for $\ell\le1.0396$ (Section~\ref{sec:wide}).

\subsection*{Mechanism 1$'$: symmetry-restricted search}

Optimizing over all quaternary forms is a nine-parameter problem, and for
indices $k\le44$ the CMA-ES search stalled below the threshold. The way out
was not a stronger optimizer but a smaller space. Requiring the lattice to
admit a prescribed finite automorphism $S\in\mathrm{GL}_4(\Z)$ turns it into a
module over an order: $\chi_S=\Phi_3^2$ or $\Phi_6^2$ makes $\Lambda$ a rank-2
$\Z[\omega]$-module (an \emph{Eisenstein} lattice), $\Phi_4^2$ a rank-2
$\Z[i]$-module, and $\Phi_m$ with $\varphi(m)=4$ a rank-1 $\Z[\zeta_m]$-module.
The space of invariant forms then has four dimensions in the rank-2 cases
(three up to scale) and only two in the rank-1 cyclotomic ones, instead of ten,
and --- crucially --- it is defined over $\Q$, so rationalizing a numerical
optimum stays inside the family. Invariant sublattices are
$\mathcal O$-submodules, so their index must be a norm; for a prime
$p\equiv1\pmod 3$ there are exactly $2(p+1)$ of them, against
$1+p+p^2+p^3$ Hermite normal forms. A dense scan of this three-parameter
family gives the record $\chi(\R^4,[1,\ell])\le43$ for $\ell\le1.00411$
(Theorem~\ref{thm:main43}), on an Eisenstein lattice with
$\mathrm{Aut}\cong\Z/6$ preserving the sublattice; the arithmetic coincidence
behind it is $43=N(7+\omega)$. The same scan, combined with an exhaustive
search over \emph{all} sublattices, finds nothing admissible for
$31\le k\le42$ (status \textbf{\cyrbadge{[Э]}}, Section~\ref{sec:open}).
...
over $E_6^*/343$). The final rung of each chain has since been recomputed in
exact arithmetic. In $\R^7$ (Theorem~\ref{thm:r7ex}) all $30\,368$ vertices
of the cell are certified over $\Z$, with completeness proved by exact edge
closure on the $1$-skeleton and without assuming the polytope is simple
($1\,600$ vertices have nine active facets). In $\R^9$
(Section~\ref{ssec:7203}) the same protocol, made feasible by an exact
unimodular change of basis, enumerates all $1\,654\,230$ vertices of the cell
with $752$ facets ($4\,590$ of them non-simple) and gives
$\diam^2V_0=1119552292542693/165294140000000<7=D_{\min}^2$. Hence both $1029$
and $7203$ have status \stC{} and enter the title; the intermediate rung
$9604$ of width $1.058$ (lamination with $m=4$) remains numerical.

\subsection*{Mechanism 3: the Eisenstein identity}
...
$|\alpha|^2=7$, index $7^{n/2}$), the identity
\[
D\bigl((3+\omega)\Lambda\bigr)^2=\tfrac73\,\lambda_1(\Lambda)^2
\]
now holds for \emph{every} Eisenstein lattice (theorem~\ref{thm:eis}, status
\stP, Section~\ref{sec:extra}). The lower bound is the \emph{planar theorem}:
the six equal-norm vectors $\pm w,\pm\omega w,\pm(1+\omega)w$ confine the
orthogonal projection of $V_0$ to the $A_2$ hexagon of $\Z[\omega]w$. The upper
bound exhibits an explicit point of $V_0$: the hexagon vertex
$q=\frac13(2w+\omega w)$ built from a minimal vector always lies in the
Voronoi cell, whatever the lattice (proposition~\ref{prop:eisup}).
Consequences: the width $\sqrt{7/6}$ of
$\chi(\R^{24},[1,\ell])\le7^{12}$ is exact rather than a lower estimate; the
$\alpha$-ladder over $\Z$, $\Z[i]$, $\Z[\omega]$ and the Hurwitz quaternions
becomes an equality (corollary~\ref{cor:Uexact}); and the impossibility of
$7^6$ in $\R^{12}$ no longer rests on an enumeration.
...
Three rigorous index screens --- the Minkowski volume screen, the inradius
screen (neither implied by the other) and the \emph{shell screen} --- bound
what any construction of this type can achieve; the first two, for instance,
forbid a flat block of index $\le16$ next to $E_8/2401$.

The shell screen (Proposition~\ref{prop:shellfloor}) strengthens the inradius
one by observing that the norm of a lattice vector ranges over shells rather
than a continuum, so that whenever the first shell above the inradius threshold
is forbidden \emph{in its entirety}, $\lambda_1(\Gamma)^2$ jumps to the next
one. Forbiddenness is certified by a one-line radial witness
(Lemma~\ref{lem:radial}: Cauchy--Schwarz together with integrality of inner
products), so the proofs are checkable by hand, with no vector enumeration at
all. On two classical parents the resulting floor \emph{coincides with the
record}: every admissible sublattice of $E_8$ has index at least $2401$
(Theorem~\ref{thm:e8opt}) and every admissible sublattice of the bcc lattice
has index at least $15$ (Theorem~\ref{thm:a3opt}) --- both known constructions
are therefore unimprovable on those parents. For $E_6^*$ the floor is $305$
against the record $343$, for $K_{12}$ it is $177\,979$, and for the Leech
lattice $5\,688\,009\,064$ against the record $7^{12}$.

Negative results are reported alongside
positive ones: unattainability of $7^6$ in $\R^{12}$, failure of rank-$2$
layers, local minimality of $\rho$ at $E_8$, both arithmetic families of index
$337$ in $\R^6$ closed exhaustively, and negative
...
\stN: $43$, $132$, $1029$, and $7203$ have status \stC, while $45619$
has status \stP. The stronger $28812$ carries \stN{} of
...
point --- which is still not a proof, and it is therefore excluded from
the title.
