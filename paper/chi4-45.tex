...
Отдельный раздел посвящён размерностям $9$--$12$. Объёмный экран Минковского
даёт строгую нижнюю границу индекса для каждой родительской решётки и
показывает, что размерности~$9$ не хватало не оптимизации, а конструкции;
таковой оказывается \emph{ламинирование} --- подъём раскраски $\R^{n-1}$ слоями,
опирающийся на два одностроч\-ных неравенства. Оно даёт
$\chi(\R^9)\le9604$ вместо $17253$ (вместе с лестницей ширин, стремящейся к
$\sqrt{7/6}$), а для $\Gamma=3\Lambda$ --- правило $d=2/\rho$ и башню
ламинаций $E_8$ с явными ширинами интервалов при $n=10,11,12$. Эти результаты
получены вычислениями с плавающей точкой поверх доказанных неравенств и
помечены соответствующим статусом.
\end{abstract}
